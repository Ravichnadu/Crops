...
import com.marolix.dhalary.exception.UserException;
...
public class CropsServiceImp implements CropsService {
	...
	public List<CropsDto> getAllCrops() {
		...
		return list.stream().map(crop->modelMapper.map(crop, CropsDto.class)).collect(Collectors.toList());
	}
	...
	public CropsDto getByCropName(String name) {
		Optional <Crops> optional= Optional.ofNullable(cropsReposistry.findByCropName(name));
		...
		if(optional .isPresent())
		{
			...
			throw new UserException("userData is Not Found : " +name);
		}
	}

	@Override
	public List<DhalaryDto> getByDhalaryLocation(String location) {
		Optional<List<Dhalary>> optional=Optional.ofNullable(dhalaryReposistry.findByLocation(location));
		if(optional .isPresent())
		{
			return optional.stream().map(dhalary->modelMapper.map(dhalary, DhalaryDto.class)).collect(Collectors.toList());
		}
		else {
			throw new UserException("userData is Not Found : " +location);
		}
